...
class CreateDatasetEvaluatorRequest(EvaluatorRequest):
    ...
    output_configs: Optional[list[EvaluatorOutputConfig]] = Field(
        default=None,
        description=(
            "Null inherits the shared output configs. Code and built-in bindings may override "
            "with any configs, including []; LLM bindings must keep at least one categorical "
            "config that matches the prompt's tool schema."
        ),
    )
    evaluator: Annotated[
...
class PatchDatasetEvaluatorRequest(EvaluatorRequest):
    ...
    output_configs: Optional[list[EvaluatorOutputConfig]] = Field(
        default=UNDEFINED,
        description=(
            "Omit to preserve; null restores inheritance. Code and built-in bindings may "
            "override with any configs, including []; LLM bindings must keep at least one "
            "categorical config that matches the prompt's tool schema."
        ),
    )

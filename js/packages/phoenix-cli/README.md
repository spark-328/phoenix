...
| `--evaluator <json>`      | Inline JSON for a new evaluator with `type` of `llm` or `code`                                                           | —        |
...
| `--output-configs <json>` | JSON array of output configurations that override the definition's (LLM evaluators: must match the prompt's tool schema) | —        |
| `--format <format>`       | `pretty`, `json`, or `raw`                                                                                               | `pretty` |

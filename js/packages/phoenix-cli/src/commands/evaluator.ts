...
interface EvaluatorCreateOptions extends CommonOptions<OutputFormat> {
  ...
  description?: string;
  /**
   * `--output-configs <json>`: JSON array of output configurations.
   */
...
async function evaluatorCreateHandler(
  ...
  const usage =
    "px evaluator create --name <name> --file evaluator.py --language PYTHON --sandbox-config-id <id> --input-mapping <json>";
...
async function evaluatorUpdateHandler(
  ...
  try {
    const patch = buildEvaluatorPatch({ type, options });
    const client = createClientOrExit(options);
...
export function createEvaluatorCreateCommand(): Command {
  return addCommonReadOptions(
    new Command("create")
      ...
      .option("--output-configs <json>", "JSON array of output configurations")
  )
    .addHelpText(
      "after",
      "\nExamples:\n" +

...
interface DatasetEvaluatorFieldOptions extends CommonOptions<OutputFormat> {
  ...
  inputMapping?: string;
  /**
   * `--output-configs <json>`: JSON array of output configurations that
   * override the shared definition's on this binding. For LLM evaluators they
   * must match the prompt's tool schema.
   */
...
interface DatasetEvaluatorCreateOptions extends DatasetEvaluatorFieldOptions {
   ...
   * `prompt_version_id` (an existing version), not both.
   */
  evaluator?: string;
  /**
...
function resolveEvaluatorInput(
  ...
  return parseJsonObjectFlag<EvaluatorInput>({
    flag: options.evaluator !== undefined ? "--evaluator" : "--evaluator-file",
...
function addBindingFieldOptions(command: Command): Command {
  return command
    ...
    .option(
      "--output-configs <json>",
      "JSON array of output configurations that override the definition's (LLM evaluators: must match the prompt's tool schema)"
    );
}
  ...
  return addCommonReadOptions(
    addBindingFieldOptions(
      new Command("create")
        ...
        .option(
          "--evaluator <json>",
          'Inline JSON for a new evaluator with "type": "llm" or "code" (LLM: give "prompt_version" or "prompt_version_id", not both)'
        )
        .option(
          "--evaluator-file <path>",
